  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 rounded-xl gradient-orange px-4 py-2.5 text-sm font-semibold text-white shadow-luxury hover:shadow-luxury-lg transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] w-fit"><Plus className="h-4 w-4" /> เพิ่มหมวดหมู่</button>
      </div>
        ...
        <div className="rounded-2xl bg-white border border-orange-100/60 shadow-luxury overflow-hidden">
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 sm:items-end">
              ...
              <button type="submit" className="rounded-xl gradient-orange px-6 py-2.5 text-sm font-semibold text-white shadow-luxury hover:shadow-luxury-lg transition-all w-full sm:w-auto">{editId ? "บันทึก" : "เพิ่ม"}</button>
            </form>
          </div>
        </div>
      )}
      ...
      <div className="sm:hidden space-y-3">
        {categories.map((c: any) => (
          <div key={c.id} className="rounded-2xl bg-white border border-orange-100/60 shadow-sm p-4">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-orange-50 border border-orange-200 flex-shrink-0">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button onClick={() => startEdit(c)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg hover:bg-orange-50 transition-colors"><Pencil className="h-4 w-4 text-orange-500" /></button>
        ...
        {categories.length === 0 && (
          <div className="text-center text-muted-foreground py-12 bg-white rounded-2xl border border-orange-100/60">
            <FolderOpen className="h-8 w-8 mx-auto mb-2 text-orange-200" />ยังไม่มีหมวดหมู่
          </div>
        )}
      </div>
        ...
        <div className="p-0 overflow-x-auto">
          <Table>
            ...
            <TableBody>
              ...
              {categories.length === 0 && (
                <TableRow><TableCell colSpan={3} className="text-center text-muted-foreground py-12"><FolderOpen className="h-8 w-8 mx-auto mb-2 text-orange-200" />ยังไม่มีหมวดหมู่</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </div>
